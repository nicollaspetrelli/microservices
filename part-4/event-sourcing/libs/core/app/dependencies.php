...
return function (ContainerBuilder $containerBuilder, array $definitions) {
    ...
    $containerBuilder->addDefinitions([
        ...
        EventStoreDb::class => function (ContainerInterface $c) {
            return new EventStoreDb(
                $c->get(EventStoreConnection::class),
                $_ENV['APP_EVENTSTORE_HTTP_HOST'] ?? 'localhost',
                (int) $_ENV['APP_EVENTSTORE_HTTP_PORT'] ?? 2113
            );
        },
        EventStoreConnection::class => function (ContainerInterface $c) {
            return EventStoreConnectionFactory::createFromUri(
                Uri::fromString($_ENV['APP_EVENTSTORE_URI'])
            );
        },
